...
def main() -> int:
    if len(sys.argv) != 4:
        print(
            "usage: compare_output.py <exe> <input.csv> <expected.csv>", file=sys.stderr
        )
        return 2
    ...
    for i, pair in enumerate(zip(actual_lines, expected_lines)):
        if pair[0] != pair[1]:
            ...
            break
    print(
        f"line counts: actual {len(actual_lines)}, expected {len(expected_lines)}",
        file=sys.stderr,
    )
    return 1
